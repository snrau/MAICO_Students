<script>
  ...
  async function testMeloPoints() {
    ...
    let matrix = drutil.distanceMatrix(
      flatten.map((melo) => melo[0]),
      $similarityweight
    );
    distMatrix.set(matrix);
    let emotionfeatures = muutil.calcEmotion(flatten.map((melo) => melo[0]));
    $emotionbased.value ? (matrix = emotionfeatures) : null;
    let mdspoints = await drutil.getPoints(matrix, false, $emotionbased.value);
    let umappoints = await drutil.getPoints(matrix, true, $emotionbased.value);
    let mdsgpoints = drutil.gridify(mdspoints, 0); // 0 hilbert, 1 gosper ???, 2 dgrid (does not work)
    let umapgpoints = drutil.gridify(umappoints, 0);
    if (
      mdspoints !== undefined &&
      ...
      const maxvalues = muutil.calcGlyphMax(flatten.map((melo) => melo[0]));
      ...
      flatten.forEach((melo, index) => {
        const sim = muutil.ourDistanceFunction(melo[0], melo[0].primer, 1, 0.5);
        const varInt = muutil.calcVariance(melo[0].notes);
        ...
        const key = muutil.getKeyfromScale(scale[0]);
        const major = muutil.getMajorfromScale(key);
        const harmonicInfo = muutil.getHarmonics(
          melo[0],
          key,
          melo[1].model.name
        ...
        let information = {
          ...
          primerindex:
            melo[0]?.primer?.index !== undefined ? melo[0]?.primer?.index : 0,
          model: melo[1].model,
          index: index,
          ...
          visdata: [
            [
              ...
              varInt,
              melo[0].notes.length,
              muutil.getInScalePercent(harmonicInfo[0]),
            ],
            [
              mdspoints[index][1],
              mdsgpoints[index][1],
          ...
          chromapie: { data: glutil.calcDataPie(melo[0]), major: major },
          ...
          additional: {
            ...
            percentagePause: muutil.percentagePauses(melo[0]),
            harmonicInfo: harmonicInfo[0],
            outScaleNotes: harmonicInfo[1],
            harmonicScore: muutil.getHarmonicScore(melo[0], harmonicInfo[0]),
            key: key,
            altscales: scale[1],
