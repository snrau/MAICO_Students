  ...
  <div class="container container1">
    <div>
      ...
      {#if devmode} -->
        ...
        <button on:click={() => mu.requestModels([allPrimer[0]])}
    ...
    <div>
      ...
      {#if filter}
        <div class="filtercontainer">
          ...
          <div class="doubleslider">
            <DoubleRangeSlider
              title={"Notes in scale"}
              values={filterinscale}
            />
          </div>
          <div class="doubleslider">
            <DoubleRangeSlider
          ...
          <div class="slider-container">
            ...
            <span style="display: inline-block; width: 30px;">
              {$filterkey === 12 ? "all" : keysLookup[$filterkey]}
            </span>
          </div>
          <div>
            <div
    ...
    <div>
      ...
      {#if calculation}
        ...
        <div class="select">
          <div>
            <input type="checkbox" bind:checked={$keymode} />
            <span>Key selection</span>
          </div>
          {#if !$keymode}
            <div>
              <label for="selectKey">Key detection</label>
              <Select
                class="select"
                id="selectKey"
                items={keydetectoptions}
                bind:value={$keydetectselect}
                clearable={false}
              />
            </div>
          {:else}
            <div>
              <input type="checkbox" bind:checked={$primerkey} />
              <span>Use Primer key</span>
            </div>
            <div>
              <input
                type="range"
                bind:value={selectkeyrange}
                on:mouseup={() => selectkey.set(selectkeyrange)}
                min="0"
                max="23"
                step="1"
              />
              <span style="display: inline-block; width: 30px;">
                {keysLookup[selectkeyrange % 12]}
                {selectkeyrange < 12 ? "major" : "minor"}
              </span>
            </div>
          {/if}
        </div>
