...
import { player, currentpoints, axisselect, keydetectselect, seen } from '../stores/stores.js'
...
export function getExtentOfMelody(melody, mode) {
  let max = 0;
  let min = 128;
...
import { allPrimer } from '../stores/globalValues.js';
...
export function playMelody(e, event, playbackline, xend, time, reset, sample) {
  ...
  if (player1 === null || player1 === undefined) {
    player1 = new mm.SoundFontPlayer('https://storage.googleapis.com/magentadata/js/soundfonts/salamander')
    player1.loadAllSamples().then(() => { console.log('playerLoaded'); player.set(player1) })
    //player.set(player1)
